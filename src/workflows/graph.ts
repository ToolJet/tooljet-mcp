...
export function validateGraph(graph: Definition, queryIds?: Set<string>) {
  ...
  for (const n of graph.nodes) {
    ...
    if (!['input', 'query', 'if-condition', 'output'].includes(n.type)) warnings.push({ code: 'unsupported_node', path: `nodes.${n.id}`, message: `Retained ${n.type} node; configuration not validated.` });
  }
  for (const edge of graph.edges) {
    ...
    const ports: Record<string, Array<string | null | undefined>> = { input: [null, undefined], query: ['success', 'failure'], 'if-condition': ['true', 'false'], output: [] };
...
export function compileGraph(current: Definition, spec: WorkflowSpec, ids?: { node_ids: Record<string, string>; edge_ids: Record<string, string> }): Compiled {
  ...
  for (const input of spec.nodes) {
    ...
    if (input.type !== 'condition') data.nodeType = input.type === 'javascript' ? 'query' : input.type;
  ...
  for (const input of spec.edges) {
    ...
    const edge = { ...old, id, source, target, sourceHandle: input.port === 'default' ? null : input.port, type: 'custom' };
