...
export function addPageTool(client: ToolJetClient): ToolDef {
  return {
    ...
    async handler(args: { app_id: string; version_id: string; name: string }) {
      try {
        return ok(await client.createPage({ appId: args.app_id, versionId: args.version_id, name: args.name }));
      } catch (err) {
        return fail(err);
      }
    },
