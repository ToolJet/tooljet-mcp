...
describe('gateway mode — requireUserSession is not satisfied by an apiUrl-only identity', () => {
  ...
  /* Old ToolJet version: no session at all. A customer_id the Gateway confirms is real bypasses this
     gate too — unattributed build, accepted deliberately until that version upgrades. */
  it('bypasses the session requirement when the Gateway confirms a real customer with no session', async () => {
