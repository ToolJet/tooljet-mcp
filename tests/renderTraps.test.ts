...
describe('render traps found in the 2026-09-12 reviews', () => {
  ...
  it('rejects a bound table whose columns ToolJet would generate itself', () => {
    const bound = (columns: unknown) => lintComponentSpec({
      type: 'Table', name: 'Todays flights',
      properties: { data: { value: '{{queries.q_flights.data}}' }, dataSourceSelector: { value: 'rawJson' }, ...(columns === undefined ? {} : { columns: { value: columns } }) },
      styles: { contentWrap: { value: '{{true}}' } },
      layouts: { desktop: { top: 200, left: 2, width: 39, height: 600 } },
    });
    // What ToolJet writes back when the model authored none: every column autogenerated, named for the field.
    const generated = ['id', 'flight_number', 'delay_code'].map((key) => ({ name: key, key, autogenerated: 'true' }));
    expect(bound(generated).errors.some((e) => e.includes('columns ToolJet generated for itself') && e.includes('flight_number'))).toBe(true);
    expect(bound(undefined).errors.some((e) => e.includes('no authored columns') && e.includes('flight_number'))).toBe(true);
    // A projected binding with authored columns is the house style and stays clean.
    const projected = lintComponentSpec({
      type: 'Table', name: 'Todays flights',
      properties: {
        data: { value: '{{queries.q_flights.data.map(r => ({flight: r.flight_number, delay: r.delay_code}))}}' },
        dataSourceSelector: { value: 'rawJson' },
        columns: { value: [
          { name: 'Flight', key: 'flight', columnType: 'string', columnSize: 150, autogenerated: false },
          { name: 'Delay code', key: 'delay', columnType: 'string', columnSize: 130, autogenerated: false },
        ] },
      },
      styles: { contentWrap: { value: '{{true}}' } },
      layouts: { desktop: { top: 200, left: 2, width: 39, height: 600 } },
    });
    expect(projected.errors.some((e) => e.includes('raw field name'))).toBe(false);
  });
});
