...
- **Statistics sizing:** a value-only tile with \`hideSecondary:true\` needs at least **12 columns** and ≈ **110–120px** height (at most three per content row). A tile with visible secondary content needs at least **18 columns** and ≈ **130–150px** height (normally two per row). Narrower KPI cards make labels/values wrap or truncate.
...
- **Field sizing:** use the catalog default **40px authored height** for TextInput, EmailInput, NumberInput, DropdownV2, DatePickerV2, and other standard single-line fields. With top alignment the rendered footprint is about **60px**; add a 10px gap for a **70px top-to-top row step**. Reserve **90–100px authored height** for a genuinely multi-line TextArea. Do not inflate a single-line field to make its value text look larger: height changes whitespace, while these component contracts expose \`labelFontSize\` but no value-font-size style.
