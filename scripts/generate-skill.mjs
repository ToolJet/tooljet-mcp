...
- **Page header:** every page starts with a 40px top gutter that matches the side gutters, so the first component (title, masthead band or toolbar) sits at top 40, never at the canvas edge; the plain-title treatment is a title Text (top ≈ 40) + one-line subtitle Text directly under it (top ≈ 80), then ~20px before content; the other treatments are chosen in Frame the page. A default-styled Text looks unfinished.
...
- **KPI tiles:** one \`Html\` strip across the content width (about 120–140px tall — pick one height for the whole app, see the stat-tile contract below) holding a CSS grid of cards, each with a muted label, a 28px bold value bound from \`{{queries.<q>.data ...}}\` (money and large counts formatted with \`toLocaleString\`, never \`'$ ' + n\`), and a one-line context. The strip is a rule, not a fixed template: the number of cards (three to six) and the column proportions follow the figures that matter for this user, not a default of four equal columns. The figure the page exists for may lead: a wider column, or an inverted card filled with \`var(--cc-primary-brand)\` and light text, when the request names the number they watch and the design brief spends a presence move on it; equal, quiet cards otherwise, and always on a plain staff tool. Use the theme's error colour on a value only when the number itself is a problem (overdue, failed). Fall back to a \`Container\` with Text children (\`showHeader:"{{false}}"\`, surface1 background, default border, radius 12) only when a tile must expose a value to other components, and use \`Statistics\` only as one full row of three or four tiles (width 13 or 9, \`hideSecondary true\`): a lone tile under a pair, or a second row, is rejected as a broken grid.
- **One stat-tile contract per app, not per page.** Before the first page is written, fix three numbers in the design brief and reuse them on every page that has a stat row: the strip/tile **height**, the **value** font size, and the **label** font size. Pages are authored in separate phases, so re-deciding these per page is the usual way an app ends up with 140px tiles and a 28px value on Home and 120px tiles with a 22px value on the next page — every tile legal, the app visibly unfinished. Build stat rows the same way everywhere too: an \`Html\` strip on one page and \`Statistics\` tiles on another cannot line up. Which figures a page shows, and how many cards, still varies by the page's job — only the sizing is fixed.
#### Charts
- **Charts:** \`properties.title ""\` with a section-heading Text above; \`markerColor\` = the theme accent (\`#2563EB\` on the standard theme; a derived theme's accent comes from its archetype row in \`references/themes.md\`, never the standard blue on a charcoal, rose or brown theme); \`styles.backgroundColor "var(--cc-surface1-surface)"\`, \`borderColor "var(--cc-default-border)"\`, \`borderRadius 12\`; two charts per row at 19 columns each (lefts 2 and 22), height ≈ 290. **Every chart is drawn with \`plotFromJson: "{{true}}"\` and a \`jsonDescription\` expression returning \`{ data, layout }\`; never a native \`type\` (native bar and line render Plotly's defaults: Verdana labels, grey grid, flat unlabeled bars, and the native pie is the rainbow palette with no legend; the linter rejects them). Draw a chart only when it says something a tile cannot: at least four categories or six time points, and values that are not all tiny counts; three bars of 5, 2 and 1 are a list, not a chart.** House bar: \`{type:'bar', x, y, marker:{color:<accent>, line:{width:0}}, text: y.map(v => String(v)), textposition:'outside', cliponaxis:false (without it the plot area cuts the tallest bar's label in half), textfont:{size:12, color:<muted>}, hovertemplate:'%{x}<br>%{y}<extra></extra>'}\` with \`layout.bargap 0.38\`, \`yaxis {rangemode:'tozero', gridcolor:<weak border>, zeroline:false, tickfont:{size:12}}\`, \`xaxis {showgrid:false, tickfont:{size:12}}\`; sort categories by value unless they are ordinal (stages, months); categories longer than about 12 characters go on horizontal bars (\`orientation:'h'\`, categories on y, \`layout.margin.l\` sized to the longest label) and one chart holds one metric, never two fields joined into a label. House line: \`{type:'scatter', mode:'lines+markers', line:{color:<accent>, width:2.5, shape:'spline', smoothing:0.6}, marker:{size:6, color:<accent>}, fill:'tozeroy', fillcolor:<accent at 10% opacity>}\`, dates on the x axis formatted \`'%d %b'\`, y from zero. Layout font family is the page's: \`font {family:'IBM Plex Sans, sans-serif', size:12, color:<muted>}\`. When the chart JSON comes from a JavaScript query (\`jsonDescription: "{{queries.q_chart.data}}"\`), that query returns the whole \`{ data: [trace], layout: {...house layout...} }\` object, never a bare array of points: a bare array draws empty axes and Plotly's default font, which is exactly the broken chart the linter and the render audit look for. That query runs from the data query's success (a lifecycle \`refreshQueryRefs\` entry) with \`runOnPageLoad false\`: on page load it would read the data query before it has answered, and the linter rejects that. The linter also parses every JavaScript query; a stray quote fails the query silently and empties everything bound to it. Shared for all of them: \`styles.padding 16\` on the Chart component (the wrapper turns it into the Plotly margin on all four sides and ignores \`layout.margin\`; the catalog default 50 leaves a 290px chart with a 190px plot, and the linter rejects anything above 24; axes add their own room for tick labels), \`styles.backgroundColor\` for the paper (\`layout.paper_bgcolor\` is ignored too), \`font {family, size:12, color:<muted>}\`, axes \`gridcolor <weak border>\`, \`zeroline false\`, \`showline false\` (Plotly cannot read CSS variables, so these are the theme's literal muted-text and weak-border hex: \`#6B7280\` and \`#F3F4F6\` on the standard theme, the derived neutral set otherwise), and \`showlegend: true\` whenever a legend is wanted (the wrapper hides it otherwise). Series palette, in order: the accent, the accent at 65% and 40% opacity (\`rgba(r,g,b,0.65)\`, \`rgba(r,g,b,0.4)\`), a darker shade of the accent for a fourth series, then the theme's muted text colour for done/neutral and its default border colour for rejected/empty. On the standard theme that is \`#2563EB, rgba(37,99,235,0.65), rgba(37,99,235,0.4), #1D4ED8, #9CA3AF, #E5E7EB\`; on the hospitality row it starts from bronze \`#B45309\`.
...
- **State columns in tables** (status, priority, SLA, receipt): chips, not coloured text and never a coloured row or cell. An \`html\` column renders \`<span style="display:inline-block;white-space:nowrap;padding:2px 10px;border-radius:999px;font-size:12px;font-weight:600;background:<tint>;color:<ink>">Open</span>\` with tints and inks from one small map: success \`#DCFCE7\`/\`#166534\`, warning \`#FEF3C7\`/\`#92400E\`, danger \`#FEE2E2\`/\`#991B1B\`, informational the theme accent at about 12% opacity with the accent as ink, neutral \`#F3F4F6\`/\`#374151\`. Conditional \`textColor\` with the theme tokens (\`var(--cc-success-systemStatus)\`, \`var(--cc-error-systemStatus)\`) is the fallback for a column that must stay plain text. Write the chip and any formatted value with \`+\` concatenation from a lookup object, never a template literal: \`\${...}\` inside a \`{{ }}\` binding produces the \`}}\` that closes it early and blanks the table.
- **Row actions and table chrome:** see the recipe and chrome defaults in \`references/tables.md\` (quiet outline row buttons, one filled primary per page, \`allowSelection:false\` and the other toggles off unless asked, \`styles.contentWrap true\` on any table with five or more columns so long values wrap instead of being cut, height at least 90 + rowsPerPage x 45 (x 60 when it wraps; add 56 when \`displaySearchBox\` or a toolbar button is on) so the last row is never sliced, columnSize values that add up to less than the table's width (a 39-column table holds about 1,090px of columns on a laptop; extra columns belong in a side panel or modal) while every column keeps a readable width (names, emails and categories 150+, money 130+, status chips 130+, dates 110+, plain text 120+; the linter rejects narrower ones, so a wide record means fewer columns, never thinner ones), money and dates formatted in the binding; a colour lookup with a fallback is parenthesised, \`(colours[r.status] || '#F3F4F6')\`, because \`a + lookup || b + c\` short-circuits and leaves an unterminated tag, which the linter now evaluates and rejects).
...
- **Kanban cards:** a card is its own 43-column canvas about 300px wide. MCP materializes the title and description children at \`left 2, width 39\` (title top 12, description top 44, description \`textSize 12\` in the muted grey); any Text or Html card child narrower than 30 columns is rejected because it cuts a name after a few characters. Keep \`openModalOnCardClick false\` unless the card modal has been browser-verified.
...
- Text must fit its box: a tile is at least 110px tall for a label, value and caption; a Kanban card's child text is at most a third of the card wide and one line; a table with more than five text columns is the full content width (\`left:2 width:39\`), and long text columns (email, description, address) get a \`columnSize\` of 180 or more or a tooltip. Column bindings name only fields the query returns: \`undefined\` or \`Invalid date\` in a cell is a wrong key, a bug rather than a cosmetic issue.
