...
- **Resolve the design source:** explicit user requirements/design system → existing or selected ToolJet theme → recognizable brand → use case, industry and audience → ToolJet Modern fallback. Preserve the selected theme unless the user asks to replace it; an empty shell is not permission to overwrite its chosen theme.
