...
- \`list_datasources(version_id)\` → \`{ datasources: [{ id, name, kind, settings_url }], connectable: ["Google Sheets 2.0", …] }\`. \`datasources\` are the connected ones; \`connectable\` names every source ToolJet CAN connect, which is how you tell an unconnected source from one ToolJet has no connector for **without guessing**. Workspace-connected sources appear automatically in existing and brand-new apps—there is **no per-app datasource attach/link step**. Use a returned \`id\` directly; if an expected source is absent, use the workspace \`datasources_url\`; if a known source fails, use its direct \`settings_url\`. ToolJet DB is \`kind: "tooljetdb"\`.
...
Workspace-connected datasources available to the current user and selected environment are automatically available to both existing and newly created apps. Do **not** look for or invent a per-app datasource linking step: after \`create_app\`, call \`list_datasources(version_id)\` and pass the returned \`id\` to \`add_queries\`. An expected source missing from \`datasources\` but present in \`connectable\` indicates the wrong workspace, insufficient permission, an unconnected source, or missing environment configuration—not a missing app attachment. A source missing from **both** is one ToolJet has no connector for: say that plainly rather than asking them to connect something that cannot be connected, and name a REST API datasource pointed at its HTTP API as the supported route. Use the returned workspace \`datasources_url\` or datasource \`settings_url\` for a user-owned repair handoff; never configure credentials or OAuth yourself.
...
>   - **Native connector, not connected** — the name is in \`connectable\` but not in \`datasources\` (Google Sheets, Airtable, PostgreSQL, HubSpot, Salesforce, and the other kinds ToolJet ships): **do not build and do not seed a placeholder.** The app's columns come from the user's own data, so invented columns presented as theirs are wrong. Leave the conversation's app untouched and unnamed. Close with three lines: what is needed (\"connect Google Sheets to this workspace\"), the exact \`datasources_url\`, and what happens next (\"then say *continue with the Orders sheet* and I will read its columns and build the table, filters and edit form\"). Never ask for a sheet link, a file, or credentials in chat; the connection is the user's.
>   - **No connector for the source** — the name is absent from \`connectable\` too (Pipedrive, Linear, a company's own API): **stop too, and say plainly that ToolJet has no native connector for it** — do not imply one exists. If the service has an HTTP API, the supported route is a **REST API datasource** pointed at it, so name that as the next step; auth is a manual setup step and you must **never handle credentials yourself**. Close by offering the alternative rather than taking it: they can ask you to build the app now against a **ToolJet DB table seeded with representative sample data**, clearly labelled as placeholder and ready to rewire once the API is connected.
...
- **typesafe** — \`{ operation: "evaluate", state: <the record or named fields to judge>, questions: <JSON map of typed questions> }\`. Jev is a decision model, not a chat model: it returns typed answers under the question ids you sent (\`data.answers.<id>.choice\`, \`.noul\` as a 0–1 probability, \`.score\` with a \`legend\`), never text. Put every independent question about the same state in one query; keep thresholds in bindings, not in the questions.
...
- **HTML where it helps.** Prefer ToolJet's native Text, Statistics, KeyValuePair, Listview and Container components, including for read-only content. Use small theme-aware Html blocks when they improve composition, clarity or styling. Keep controls, charts, tables and interactions native, and weigh presentation against visual editability. See references/ui-layout.md for examples and safeguards.
