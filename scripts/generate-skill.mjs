...
- **Row actions and table chrome:** see the recipe and chrome defaults in \`references/tables.md\` (quiet outline row buttons, one filled primary per page, \`allowSelection:false\` and the other toggles off unless asked, \`styles.contentWrap true\` on any table with five or more columns so long values wrap instead of being cut, money and dates formatted in the binding).
