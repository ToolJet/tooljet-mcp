...
**Batch for the build, singular for edits.** Plan first, await \`lint_app_spec\` as a standalone barrier, inspect the result, and only then use \`create_tables\` / \`insert_rows_batch\` / \`add_pages\` / \`add_queries\`, one \`add_component_batches\` call for two or more ready pages (or \`add_components\` for one page), one app-wide \`add_events\` call, and at most one \`add_query_lifecycles\` call. The linter must never be a sibling of those writes in a parallel tool call. Use singular tools afterwards for incremental edits. Component/query/event batches are atomic; cross-page components and multi-table creation cannot be atomic because ToolJet has no matching bulk endpoint.
...
4. Add all ready pages with one \`add_component_batches\` call (or one \`add_components\` call for a single page); after all ids exist, add ordinary events once and lifecycle flows once.
