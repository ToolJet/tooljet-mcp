...
- `run_queries({ query_ids, version_id })` executes up to ten **proven read-only** ToolJet DB/SQL reads concurrently after preflighting the whole batch. Use it when two or more independent safe reads need real response shapes; it refuses mutations, RunJS, paid/remote APIs, and unknown kinds before executing anything.
