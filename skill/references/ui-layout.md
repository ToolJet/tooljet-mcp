...
- **Text indent:** MCP gives a `Text` a default `styles.textIndent {{10}}` so labels, sub-headers and view-only one-liners aren't flush against their left edge. It applies only to **left-aligned, single-line** Text — `textIndent` is CSS `text-indent`, which indents only the first line, so on a wrapping paragraph the remaining lines would sit ~10px left of every neighbour and read as broken alignment. MCP therefore skips it for centred/right text, `dynamicHeight` text, a box tall enough for a second line, and prose (over ~120 chars or containing newlines/`<p>`/`<br>`). Don't hand-set `{{10}}` on a wrapping paragraph for the same reason; Text has no numeric left-padding style, so indent a real paragraph block with a leading spacer column instead.
